...
for (const [col, def] of [['source', "TEXT DEFAULT 'manual'"], ['sync_closed', 'INTEGER DEFAULT 0'], ['customer', 'TEXT'], ['ss_row_id', 'TEXT'], ['geo_error', 'TEXT']]) {
  if (!db.prepare(`SELECT 1 FROM pragma_table_info('loads') WHERE name = ?`).get(col)) db.exec(`ALTER TABLE loads ADD COLUMN ${col} ${def}`);
}
if (!db.prepare("SELECT 1 FROM pragma_table_info('bids') WHERE name = 'token'").get()) db.exec('ALTER TABLE bids ADD COLUMN token TEXT');
db.exec('CREATE INDEX IF NOT EXISTS bids_token ON bids(token)');
