...
function extractCarriers(rows, mcColumnSetting) {
  ...
  let headerRow = -1, mcCol = -1, nameCol = -1;
  ...
  if (mcCol >= 0) {
    rows[headerRow].forEach((h, i) => { if (nameCol < 0 && i !== mcCol && NAME_HEADER.test(String(h))) nameCol = i; });
  } else {
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    if (mc && !out.has(mc)) out.set(mc, nameCol >= 0 ? String(rows[r][nameCol] || '').trim() : '');
  }
  return out;
}
...
function saveCarriers(map, sourceLabel) {
  ...
  try {
    ...
    const ins = db.prepare('INSERT INTO qualified_carriers (mc, name) VALUES (?, ?)');
    for (const [mc, name] of map) ins.run(mc, name);
    db.exec('COMMIT');
...
module.exports = { normalizeMC, toDownloadUrl, refreshFromUrl, importFromFile, status, check, startAutoRefresh, extractCarriers };
